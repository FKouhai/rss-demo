...
func TestPushNotificationHandler(t *testing.T) {
	payload := []byte(`{"feed_url":"https://www.reddit.com/r/sre/comments/1meh785/a_developer_wants_you_to_deploy_their_application/","webhook_url":"https://discord.com/api/webhooks/1400936674522828892/rEYQaa9DRY9pKse9YPT4XjWQmDOd6gARpp6OdYU27icRoucGCkGs3zhdlxhNuOucSiSZ"}`)
	...
	if requestRecorder.Code != http.StatusNoContent {
		t.Errorf("got %v , was expecting %v", requestRecorder.Code, http.StatusOK)
	}
}
...
func TestPushNotificationNoFeed(t *testing.T) {
	payload := []byte(`{"feed_url":"","webhook_url":"https://discord.com/api/webhooks/1400936674522828892/rEYQaa9DRY9pKse9YPT4XjWQmDOd6gARpp6OdYU27icRoucGCkGs3zhdlxhNuOucSiSZ"}`)
	requestRecorder := httptest.NewRecorder()
	req, err := http.NewRequest("POST", "/push", bytes.NewBuffer(payload))
	req.Header.Add("Content-Type", "application/json")
	if err != nil {
